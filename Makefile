...
$(BIOCRO_LIB): $(BIOCRO_SRC)/$(BIOCRO_LIB)
	cp $< $@

$(BIOCRO_SRC)/$(BIOCRO_LIB):
	echo "Build BioCro before running these tests."

test_repeat_runs: test_repeat_runs.cpp $(BIOCRO_LIB)
	clang++ -std=c++11 -I $(BIOCRO_SRC) $(BIOCRO_LIB) test_repeat_runs.cpp -o test_repeat_runs -lgtest_main -lgtest

test_dynamical_system: test_dynamical_system.cpp $(BIOCRO_LIB)
	clang++ -std=c++11 -I $(BIOCRO_SRC) $(BIOCRO_LIB)  test_dynamical_system.cpp -o test_dynamical_system -lgtest_main -lgtest

test_biocro: test_biocro.cpp $(BIOCRO_LIB)
	clang++ -std=c++11 -I $(BIOCRO_SRC) $(BIOCRO_LIB) test_biocro.cpp -o test_biocro -lgtest -lgtest_main

test_harmonic_oscillator: test_harmonic_oscillator.cpp $(BIOCRO_LIB)
	clang++ -std=c++11 -I $(BIOCRO_SRC) $(BIOCRO_LIB) test_harmonic_oscillator.cpp -o test_harmonic_oscillator -lgtest -lgtest_main

segfault_test: segfault_test.cpp
	clang++ -std=c++11 segfault_test.cpp -o segfault_test -lgtest
